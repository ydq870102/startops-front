...
export default {
  ...
  methods: {
    ...
    // 获取设备列表
    getAllHost() {
      if (Array.isArray(this.host_list) && this.host_list.length === 0) {
        get('api/cmdb/host/', { size: 10000 }).then(res => {
          const _this = this
          res.results.forEach(function(data, index) {
            const host = { value: data.id, label: data.label_cn }
            _this.host_list.push(host)
          })
        })
      }
    },
