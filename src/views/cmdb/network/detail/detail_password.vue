<template>
  <div class="app-container">
    <div style="display: inline-block;margin: 10px 14px;float: left">
      <el-button
        v-if="checkPermission(['admin','netuser_all','netuser_create'])"
        size="mini"
        type="primary"
        icon="el-icon-plus"
    ...
    <el-table v-loading="loading" ref="table" :data="data" size="small" border style="width: 100%;">
      ...
      <el-table-column prop="related_network_id.label_cn" label="交换机名称" width="180"/>
      <el-table-column prop="related_network_id.ip" label="交换机IP" show-overflow-tooltip width="120"/>
      ...
      <el-table-column prop="is_3a" label="是否3A" show-overflow-tooltip />
      <el-table-column prop="is_en" label="是否EN" show-overflow-tooltip />
      <el-table-column prop="enpassword" label="en密码" show-overflow-tooltip />
      <el-table-column prop="remark" label="备注" show-overflow-tooltip />
      <el-table-column label="操作" align="center" min-width="160">
        <template slot-scope="scope">
          <el-button v-if="checkPermission(['admin','netuser_all'])" size="mini" type="success" @click="toOpenDialog(scope.row)">编辑</el-button>
          <el-popover
            v-if="checkPermission(['admin','netuser_all'])"
            :ref="scope.row.id"
            placement="top"
            width="180">
            <p>确定删除本条数据吗？</p>
